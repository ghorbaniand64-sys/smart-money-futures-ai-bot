...
║  V17.8.0 — LIQUIDITY MAP + REACTION + EARLY IMPULSE ENGINE                                          ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  RELEASE: V17.8.0-LIQUIDITY-MAP-EARLY-IMPULSE                                   ║
║                                                                              ║
║  PURPOSE                                                                     ║
...
export const BOT_VERSION = "V17.8.0-LIQUIDITY-MAP-EARLY-IMPULSE";
export const BOT_BUILD = "V17.8.0-LIQUIDITY-MAP-EARLY-IMPULSE";
...
const CONFIG = {
VERSION: "V17.8.0-LIQUIDITY-MAP-EARLY-IMPULSE",
MODE: "SIGNAL",
...
function hybridEarlyImpulse(c5,price,atrValue,flow,vol,resistance,support){
  if(!CONFIG.HYBRID_ENTRY.earlyImpulseEnabled||!(price>0)||!(atrValue>0))return {long:false,short:false,evidence:[]};
  const cur=c5.at(-1),prev=c5.at(-2);if(!cur||!prev)return {long:false,short:false,evidence:[]};const m=hybridCandleMetrics(cur);
  const move1=hybridPercentChange(c5,1),prevMove=hybridPercentChange(c5.slice(0,-1),1),acc=move1-prevMove;
  const bull=m.bullish&&m.bodyRatio>=.55&&move1>0&&acc>=.06,bear=m.bearish&&m.bodyRatio>=.55&&move1<0&&acc<=-.06;
  const volOk=vol.volumeAvailable?vol.volumeRatio>=1.45:vol.rangeRatio>=1.25,fl=hybridFlowAligned(flow,'LONG'),fs=hybridFlowAligned(flow,'SHORT'),flowL=fl>=.10,flowS=fs>=.10;
  const nearR=resistance&&Math.abs(price-Number(resistance.center))/atrValue<=.30,nearS=support&&Math.abs(price-Number(support.center))/atrValue<=.30;
  const brokeR=resistance&&price>Number(resistance.high),brokeS=support&&price<Number(support.low);
  const w=[];
  for(let i=Math.max(19,c5.length-20);i<c5.length;i++){const xs=c5.slice(i-19,i+1).map(x=>Number(x.close)).filter(Number.isFinite);if(xs.length<20)continue;const mu=xs.reduce((a,b)=>a+b,0)/xs.length;const sd=Math.sqrt(xs.reduce((a,x)=>a+(x-mu)**2,0)/xs.length);if(mu>0)w.push(4*sd/mu*100);}
  const bbNow=w.at(-1)||0,bbPrev=w.at(-2)||bbNow;
  const squeeze=bbPrev>0&&bbPrev<=2.5&&bbNow>=bbPrev*1.08;
  const long=bull&&volOk&&flowL&&(nearR||brokeR||squeeze)&&Math.abs(move1)/Math.max(atrValue/price*100,.01)<=1.10;
  const short=bear&&volOk&&flowS&&(nearS||brokeS||squeeze)&&Math.abs(move1)/Math.max(atrValue/price*100,.01)<=1.10;
  return {long,short,evidence:[bull?'EARLY_BULLISH_DISPLACEMENT':bear?'EARLY_BEARISH_DISPLACEMENT':null,volOk?'VOLUME_ACCELERATION':null,flowL?'BUY_FLOW_ACCELERATION':null,flowS?'SELL_FLOW_ACCELERATION':null,squeeze?'VOLATILITY_SQUEEZE_RELEASE':null,brokeR?'RESISTANCE_FIRST_BREAK':null,brokeS?'SUPPORT_FIRST_BREAK':null].filter(Boolean),move1,acc,squeeze};
}
...
function hybridClassifyStructure(symbol,candles,price,flow,previousState={}){
  ...
  if(newCandle){
    const early=hybridEarlyImpulse(c5,price,atr5,flow,vol,resistance,support);
    if(early.long) entries.push({direction:'LONG',trigger:'EARLY_IMPULSE_LONG',zone:resistance||support||null,evidence:early.evidence,atr:atr5,price,volume:vol,flow,early:true,entryDistanceAtr:resistance?Math.abs(price-Number(resistance.center))/atr5:0,tier:'FAST',confirmationCount:3});
    else if(early.short) entries.push({direction:'SHORT',trigger:'EARLY_IMPULSE_SHORT',zone:support||resistance||null,evidence:early.evidence,atr:atr5,price,volume:vol,flow,early:true,entryDistanceAtr:support?Math.abs(price-Number(support.center))/atr5:0,tier:'FAST',confirmationCount:3});
  }
  ...
  if(!next.stage && newCandle && support){
  ...
  if(!next.stage && newCandle && resistance){
...
async function runFullScan(env, scanOptions = {}) {
  ...
  for(const row of rows){
    ...
    try{
      ...
      let setup=hybridBuildSetup(row.s,analysis,flow,traders);
      const setupMode="LIQUIDITY_REACTION_5M";
      if(setup && !String(setup.trigger||"").includes("REACTION_PULLBACK_RETEST") && !String(setup.trigger||"").includes("EARLY_IMPULSE_")) setup=null;
      if(setup){
        ...
        signals.push({
          ...
          diagnostics:setup.evidence, signalTier:"LIQUIDITY_REACTION_5M", executionEligible:true, edge:0, riskScore:0,
          entryQuality:{overextended:false}, entryEngine:"LIQUIDITY_REACTION_5M", eventStrength:0, topTraderIntelligence:null,
