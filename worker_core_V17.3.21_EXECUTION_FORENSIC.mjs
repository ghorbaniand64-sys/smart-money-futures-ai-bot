...
function findBigIntPaths(value, path = "$", out = [], seen = new WeakSet()) {
  if (typeof value === "bigint") {
    out.push({ path, digits: value.toString().length, tail: value.toString().slice(-16) });
    return out;
  }
  if (value === null || value === undefined || (typeof value !== "object" && typeof value !== "function")) return out;
  if (typeof value === "object") {
    if (seen.has(value)) return out;
    seen.add(value);
  }
  if (Array.isArray(value)) {
    for (let i = 0; i < value.length; i++) findBigIntPaths(value[i], `${path}[${i}]`, out, seen);
    return out;
  }
  for (const key of Object.keys(value)) {
    try { findBigIntPaths(value[key], `${path}.${key}`, out, seen); } catch (_) {}
  }
  return out;
}

function executionStageDiagnostic(stage, error, extra = {}) {
  const message = safeError(error);
  const result = {
    stage,
    message,
  ...
  console.error("[GMX][EXECUTION_FORENSIC]", result);
  return result;
}

async function executeExpressOrderDiagnostic(sdk, request, signer, meta = {}) {
  // V17.3.21: FORENSIC execution tracing. Native BigInt is deliberately
  // preserved for every GMX SDK call. We only inspect object paths after a
  // failure, so this diagnostic cannot itself trigger BigInt serialization.
  const requestBigInts = findBigIntPaths(request);
  console.log("[GMX][EXECUTION_FORENSIC][REQUEST]", {
    ...meta,
    bigintPaths: requestBigInts,
    bigintCount: requestBigInts.length,
  });

  let prepared;
  try {
    prepared = await sdk.prepareOrder(request);
    console.log("[GMX][EXECUTION_FORENSIC][PREPARE_OK]", {
      requestId: prepared?.requestId || null,
      payloadType: prepared?.payloadType || null,
      preparedBigInts: findBigIntPaths(prepared).length,
    });
  } catch (error) {
    const diagnostic = executionStageDiagnostic("EXPRESS_PREPARE", error, {
      ...meta,
      requestBigInts,
      request: summarizeExpressValue(request),
    });
    throw executionStageError("EXPRESS_PREPARE", error, diagnostic);
  }

  let signature;
  try {
    signature = await sdk.signOrder(prepared, signer);
    console.log("[GMX][EXECUTION_FORENSIC][SIGN_OK]", {
      requestId: prepared?.requestId || null,
      signatureType: typeof signature,
      preparedBigInts: findBigIntPaths(prepared).length,
    });
  } catch (error) {
    const diagnostic = executionStageDiagnostic("EXPRESS_SIGN", error, {
      ...meta,
      requestId: prepared?.requestId || null,
      payloadType: prepared?.payloadType || null,
      preparedBigInts: findBigIntPaths(prepared).length,
      preparedBigIntPaths: findBigIntPaths(prepared),
      payload: summarizeExpressValue(prepared?.payload),
    });
    throw executionStageError("EXPRESS_SIGN", error, diagnostic);
  }
  ...
  const submitBigInts = findBigIntPaths(submitRequest);
  console.log("[GMX][EXECUTION_FORENSIC][SUBMIT_INPUT]", {
    requestId: prepared?.requestId || null,
    bigintPaths: submitBigInts,
    bigintCount: submitBigInts.length,
  });

  let submitted;
  try {
    submitted = await sdk.submitOrder(submitRequest);
    console.log("[GMX][EXECUTION_FORENSIC][SUBMIT_OK]", {
      requestId: submitted?.requestId || prepared?.requestId || null,
      status: submitted?.status || null,
      responseBigInts: findBigIntPaths(submitted).length,
    });
  } catch (error) {
    const diagnostic = executionStageDiagnostic("EXPRESS_SUBMIT", error, {
      ...meta,
      requestId: prepared?.requestId || null,
    ...
    throw executionStageError("EXPRESS_SUBMIT", error, diagnostic);
  }
