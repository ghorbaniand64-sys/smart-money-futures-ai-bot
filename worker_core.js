...
// Version: V15.6.4 / Phase 6 GitHub Actions Adapter Multi-Source Smart Money + Independent Radar + Smart Money Data Health + OI/Funding Integrity + Volume-Aware OHLCV + Exploration Deep Scan + Radar Coverage
...
 
const CONFIG = {
VERSION: "V15.6.4-GITHUB-ACTIONS-PHASE6",
MODE: "SIGNAL",
...
function radarTickerPrice(row){
  ...
  const raw=row.medianPrice ?? row.price ?? row.markPrice ?? row.indexPrice ?? row.oraclePrice ?? row.maxPrice ?? row.minPrice;
  if(raw===null || raw===undefined || raw==="") return 0;
  const n=Number(raw);
  if(!Number.isFinite(n) || n<=0) return 0;
  // GMX Oracle /prices/tickers prices are 30-decimal USD values.
  // Normalize them to ordinary USD price before Radar consumes them.
  if(n>1e12) return n/1e30;
  return n;
}
...
const radarMarkets=allMarkets.map(m=>{
  ...
  const livePx=Number(radarPriceFeed?.prices?.[sym]||0);
  return livePx>0 ? {...m,price:livePx,markPrice:livePx,indexPrice:livePx} : m;
});
...
const price = finitePositive(market?.price ?? market?.markPrice ?? market?.indexPrice ?? market?.currentPrice ?? market?.indexPriceUsd);
...
const stopPct = radarStopPercent(candidate);
...
const move = position.side === "LONG" ? (price - entry) / entry : (entry - price) / entry;
return notional * (Number(percent) / 100) * move;
}
 
...
const currentPrice = Number(snapshot?.price || snapshot?.currentPrice || 0);
