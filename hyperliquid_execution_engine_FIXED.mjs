...
const MAX_HANDOFF_AGE_MS = Number(process.env.EXECUTION_HANDOFF_TTL_MS || 360000); // 6 min default; revalidation gates still protect stale entries
...
function handoffCreatedAtMs(handoff){
  const raw=handoff?.createdAt;
  if(raw===null||raw===undefined||raw==='')return NaN;
  if(typeof raw==='number' || /^\d+(?:\.\d+)?$/.test(String(raw).trim())){
    const n=Number(raw);
    if(!Number.isFinite(n))return NaN;
    // Accept Unix seconds as well as JavaScript milliseconds.
...
async function buildCandidate(handoff){
  ...
  const now=Date.now();
  const createdAtMs=handoffCreatedAtMs(handoff);
  if(!Number.isFinite(createdAtMs))throw new Error('HANDOFF_CREATED_AT_INVALID');
  const age=now-createdAtMs;
  if(age < -30000)throw new Error('HANDOFF_CREATED_AT_IN_FUTURE');
  if(age>MAX_HANDOFF_AGE_MS)throw new Error('HANDOFF_EXPIRED');
