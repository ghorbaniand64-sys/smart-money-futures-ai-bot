...
const CONFIG = {
VERSION: "V17.3.11-CANONICAL-COLLATERAL-TOKEN",
MODE: "SIGNAL",
...
${icon2} EXECUTED #${i+1} — ${telegramTextSafe(x?.symbol || "UNKNOWN")}`,
  ...
  return [
    ...
    result?.allowancePreflight ? `🔐 Allowance preflight: ${result.allowancePreflight.attempted ? (result.allowancePreflight.ok ? "READY" : "ISSUE") : "NOT_RUN"} | Approved ${Number(result.allowancePreflight.approved?.length||0)} | Existing ${Number(result.allowancePreflight.sufficient?.length||0)} | Errors ${Number(result.allowancePreflight.errors?.length||0)} | ${Number(result.allowancePreflight.elapsedMs||0)}ms` : null,
