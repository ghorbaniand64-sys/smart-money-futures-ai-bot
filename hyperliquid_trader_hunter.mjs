...
function memeProfile(trades){
  ...
  // V8.4: concentration must measure the share of Meme PnL attributable to each
  // normalized coin, not the coin's net PnL divided by all trade-level gross P/L.
  // The old denominator could make a single-coin trader appear only 25% concentrated
  // when that one coin had large internal wins/losses. One unique coin must read 100%.
  const topPnl=[...coinPnl.values()].sort((a,b)=>Math.abs(b)-Math.abs(a));
  const netAbsByCoin=[...coinPnl.values()].reduce((a,v)=>a+Math.abs(Number(v||0)),0);
...
function compactTelegramReport({d,scanned,top,promotionTop,observationCount,cycle}){
  const header=['🟣 HYPERLIQUID MEME HUNTER V8.4','📡 READ-ONLY | NO ORDERS','━━━━━━━━━━━━━━━━━━',`🔁 Promotion cycle: ${cycle}`,`🎯 FINAL COPY CANDIDATES: ${top.filter(x=>x.executionReady).length} | Strict specialists: ${scanned.length}`,`🔬 Promotion / Research tracked: ${promotionTop.length} | Observation excluded: ${observationCount}`];
...
async function main(){
  ...
  const errors=[];
  console.log(`[MEME-HUNTER V8.4][START] mode=${MEME_MODE} watchlist=${MEME_WATCHLIST.length}`);
  let d;
  try{d=await discover()}catch(e){
    console.error(`[DISCOVERY][ERROR] ${e.message}`);
    await telegram(`🟣 HYPERLIQUID MEME HUNTER V8.4\n📡 READ-ONLY | NO ORDERS\n━━━━━━━━━━━━━━━━━━\n❌ DISCOVERY ERROR\n${e.message}`);process.exitCode=1;return;
  }
  ...
  lines.push('','ℹ️ V8.4 validates realized Meme PnL/WR/PF plus entry timing, exit capture and post-exit continuation on the sampled recent Meme trades.' ,'ℹ️ Early/exit behavior describes repeated historical execution; it does NOT establish advance knowledge of future pumps/dumps.','ℹ️ Watchlist contains the top five available research candidates; Copyability is shown separately from Meme specialization.','ℹ️ V8.4 keeps Strict Multi-Meme Specialist separate from Concentrated Meme behavior; neither research tier redefines strict eligibility.','ℹ️ Probable/unknown symbols never count toward specialist eligibility.','ℹ️ No orders are created by this worker.',`🕐 ${new Date().toISOString()}`);
  ...
  console.log(`[MEME-HUNTER V8.4][DONE] discovered=${d.discovered} scanned=${sourceAddresses.length} specialists=${scanned.length} top=${top.length} errors=${errors.length} seconds=${((Date.now()-t0)/1000).toFixed(1)}`);
  await telegram(compactTelegramReport({d,scanned,top,promotionTop,observationCount:Math.max(0,allResearch.length-promotionResearch.length),cycle}));
}

main().catch(async e=>{console.error(`[MEME-HUNTER V8.4][FATAL] ${e.stack||e}`);await telegram(`🟣 HYPERLIQUID MEME HUNTER V8.4\n📡 READ-ONLY | NO ORDERS\n━━━━━━━━━━━━━━━━━━\n💥 FATAL ERROR\n${String(e.message||e).slice(0,1000)}`);process.exitCode=1});
