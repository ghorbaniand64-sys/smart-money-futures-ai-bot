...
async function main(){
 console.log(`[HUNTER V5.12][START] ${JSON.stringify({lookbackDays:LOOKBACK_DAYS,maxCandidates:MAX_CANDIDATES})}`);
 ...
 for(const x of passed.slice(0,5)){try{const p=await position(x.address);if(!p){x.enrichmentBlock='NO_OPEN_POSITION';continue}const coin=String(p.coin||''),bk=await book(coin),av=await atr(coin,now),pl=plan(p,bk,av);pl.coin=coin;x.plan=pl;if(pl.eligible)selected.push(x)}catch(e){x.enrichmentBlock=`${category(e)}:${e.message}`;console.error(`[ENRICH][ERROR] ${short(x.address)} ${x.enrichmentBlock}`)}}
 ...
 const lines=['🟣 HYPERLIQUID TRADER HUNTER V5.12','📡 READ-ONLY | NO ORDERS','━━━━━━━━━━━━━━━━━━',`🔎 Discovery: ${d.discovered}`,`👥 Candidates: ${d.candidates.length}/${MAX_CANDIDATES}`,`📊 Scanned OK: ${scanned.length}/${d.candidates.length}`,`⚠️ Scan failures: ${errors.length}`,`❌ Error types: ${errSummary(errors)}`,`🎯 Statistical PASS: ${passed.length}`,`🧱 Top filter blocks: ${top}`,`📚 Lookback: ${LOOKBACK_DAYS}d | Hyperliquid retains a finite recent fill history`,'','🏆 SELECTED TRADERS'];
 if(!selected.length)lines.push('No trader currently satisfies the statistical filters + open-position + copy-plan checks.');else selected.forEach((x,i)=>{const m=x.metrics,p=x.plan;lines.push('',`#${i+1} ${short(x.address)}`,`👤 Trader: ${x.address}`,`🔎 Tracker: https://app.hyperliquid.xyz/explorer/address/${x.address}`,`🪙 Symbol: ${p.coin||'n/a'}`,`📈 7D: trades=${m.closedTrades} | WR=${pct(m.winRate)} | PnL=${fmt(m.pnl)} | PF=${fmt(m.profitFactor)}`,`⏱ Hold: median=${fmt(m.medianHoldHours)}h | avg=${fmt(m.avgHoldHours)}h | activeDays=${m.activeDays}`,`🧯 Streak=${m.maxLosingStreak} | liq=${m.liquidations} | fills=${m.fills}`,`📌 Copy-plan: ${p.side} | source=${fmt(p.sourceEntry)} | now=${fmt(p.entry)} | dist=${pct(p.distancePct,2)} | SL=${fmt(p.sl)} | TP=${fmt(p.tp)} | RR=${fmt(p.rr)} | maxHold=${p.maxHoldHours}h`,`ℹ️ Diagnostic TP/SL only | Source TP/SL not copied`)});
 ...
 lines.push('','ℹ️ Entry/SL/TP are READ-ONLY copy-plan diagnostics.','ℹ️ Source TP/SL is not copied.','ℹ️ No orders are created by this worker.',`🕐 ${new Date().toISOString()}`);console.log(`[HUNTER V5.12][DONE] discovered=${d.discovered} candidates=${d.candidates.length} scanned=${scanned.length} failed=${errors.length} pass=${passed.length} selected=${selected.length}`);await telegram(lines.join('\n'));
}
