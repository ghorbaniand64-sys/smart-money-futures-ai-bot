// Hyperliquid Meme Specialist Scout V5.33 - READ ONLY
...
// V5.33: strict specialist eligibility remains unchanged. Focus is a broader
// research tier, while Research Near-Miss isolates complete-history traders that
// are materially closer to the strict gate and deserve early-move analysis.
...
const MEME_FOCUS_MAX_DOMINANT_PCT = num('HYPERLIQUID_MEME_FOCUS_MAX_DOMINANT_PCT', 85);
const MEME_FOCUS_TOP_N = integer('HYPERLIQUID_MEME_FOCUS_TOP_N', 5);
const MEME_NEARMISS_MIN_EXPOSURE = num('HYPERLIQUID_MEME_NEARMISS_MIN_EXPOSURE_PCT', 15);
const MEME_NEARMISS_MIN_TRADES = integer('HYPERLIQUID_MEME_NEARMISS_MIN_TRADES', 100);
const MEME_NEARMISS_MIN_UNIQUE = integer('HYPERLIQUID_MEME_NEARMISS_MIN_UNIQUE_COINS', 3);
const MEME_NEARMISS_TOP_N = integer('HYPERLIQUID_MEME_NEARMISS_TOP_N', 5);
...
function recentMemeProfile(fills){
  ...
  for(const f of meme){const c=normalizeMemeSymbol(f?.coin);if(c)counts.set(c,(counts.get(c)||0)+1)}
  ...
  const countScore=Math.min(100,meme.length/Math.max(1,MEME_FOCUS_MIN_TRADES)*100);
  const breadth=Math.min(100,unique/Math.max(1,MEME_FOCUS_MIN_UNIQUE)*100);
  const diversity=Math.max(0,100-Math.min(100,dominantPct));
  const score=Math.round(.40*exposure+.20*countScore+.15*breadth+.15*diversity+.10*recency);
  return {memeTrades:meme.length,totalTrades:total,exposurePct:exposure,uniqueCoins:unique,dominantMeme:dominant[0],dominantMemeTrades:dominant[1],dominantPct,score};
}
...
async function fastMemePrefilter(addresses){
  ...
  const byFocus=[...sorted].sort((a,b)=>{
    const fa=(Math.min(100,a.profile.exposurePct/Math.max(1,MEME_FOCUS_MIN_EXPOSURE)*100)*.45)+(Math.min(100,a.profile.memeTrades/Math.max(1,MEME_FOCUS_MIN_TRADES)*100)*.20)+(Math.min(100,a.profile.uniqueCoins/Math.max(1,MEME_FOCUS_MIN_UNIQUE)*100)*.20)+(Math.max(0,100-Math.min(100,a.profile.dominantPct))*0.15);
    const fb=(Math.min(100,b.profile.exposurePct/Math.max(1,MEME_FOCUS_MIN_EXPOSURE)*100)*.45)+(Math.min(100,b.profile.memeTrades/Math.max(1,MEME_FOCUS_MIN_TRADES)*100)*.20)+(Math.min(100,b.profile.uniqueCoins/Math.max(1,MEME_FOCUS_MIN_UNIQUE)*100)*.20)+(Math.max(0,100-Math.min(100,b.profile.dominantPct))*0.15);
    return fb-fa||b.profile.score-a.profile.score;
  });
  ...
  add(byCount,Math.ceil(target*.10));
  add(byFocus,Math.ceil(target*.20));
...
async function analyzeMemeTrader(x,now){
  ...
  if(incomplete && MEME_REQUIRE_COMPLETE_HISTORY)return {...x,meme:p,early:{score:0,count:0,pump:0,dump:0},memeEligible:false,focusEligible:false,dataQualityGate:false};
  const strictEligible=p.memeTrades>=MEME_MIN_TRADES&&p.exposurePct>=MEME_MIN_EXPOSURE&&p.uniqueCoins>=MEME_MIN_UNIQUE;
  const focusEligible=p.memeTrades>=MEME_FOCUS_MIN_TRADES&&p.exposurePct>=MEME_FOCUS_MIN_EXPOSURE&&p.uniqueCoins>=MEME_FOCUS_MIN_UNIQUE&&p.dominantPct<=MEME_FOCUS_MAX_DOMINANT_PCT;
  const researchNearMissEligible=!strictEligible&&!incomplete&&p.memeTrades>=MEME_NEARMISS_MIN_TRADES&&p.exposurePct>=MEME_NEARMISS_MIN_EXPOSURE&&p.uniqueCoins>=MEME_NEARMISS_MIN_UNIQUE;
  if(!strictEligible&&!focusEligible&&!researchNearMissEligible)return {...x,meme:p,early:{score:0,count:0,pump:0,dump:0},memeEligible:false,focusEligible:false,researchNearMissEligible:false,dataQualityGate:!incomplete};
  ...
  return {...x,meme:p,early:{...early,repeatability,score:edge},memeEligible:strictEligible,focusEligible,researchNearMissEligible,memeFocusScore:Math.max(0,Math.min(100,focusScore))};
}
...
async function main(){
  ...
  const errors=[];
  console.log(`[MEME-SCOUT V5.33][START] mode=${MEME_MODE} watchlist=${MEME_WATCHLIST.length}`);
  let d;
  try{d=await discover()}catch(e){
    console.error(`[DISCOVERY][ERROR] ${e.message}`);
    await telegram(`🟣 HYPERLIQUID MEME SPECIALIST SCOUT V5.33\n📡 READ-ONLY | NO ORDERS\n━━━━━━━━━━━━━━━━━━\n❌ DISCOVERY ERROR\n${e.message}`);process.exitCode=1;return;
  }
  ...
  for(let i=0;i<fullHistoryAddresses.length;i++){
    ...
    try{
      ...
      if(y.researchNearMissEligible)researchNearMisses.push(y);
  ...
  const nearMissResearchTop=[...new Map(researchNearMisses.map(x=>[x.address,x])).values()].sort((a,b)=>(b.meme?.exposurePct||0)-(a.meme?.exposurePct||0)||(b.meme?.memeTrades||0)-(a.meme?.memeTrades||0)||(b.meme?.uniqueCoins||0)-(a.meme?.uniqueCoins||0)).slice(0,MEME_NEARMISS_TOP_N);
  ...
  lines.push('',`🟠 MEME-FOCUS CANDIDATES (${focusTop.length})`,`Criteria: exposure>=${MEME_FOCUS_MIN_EXPOSURE}% | meme trades>=${MEME_FOCUS_MIN_TRADES} | unique memes>=${MEME_FOCUS_MIN_UNIQUE} | dominant<=${MEME_FOCUS_MAX_DOMINANT_PCT}%`);
  if(focusTop.length){focusTop.forEach((x,i)=>{const p=x.meme||{},e=x.early||{};lines.push(`#${i+1} ${x.address}`,`🧬 exposure=${pct(p.exposurePct,1)} | memeTrades=${p.memeTrades}/${p.totalTrades} | unique=${p.uniqueCoins}`,`🎯 dominant=${p.dominantMeme||'n/a'} (${p.dominantMemeTrades||0} trades, ${pct(p.dominantPct,1)}%) | focus=${x.memeFocusScore||0}/100`,`🚀 early=${e.score||0}/100 | hit +5=${pct(e.hit5,0)} | +10=${pct(e.hit10,0)} | +20=${pct(e.hit20,0)} | lead5=${fmt(e.medianLead5,1)}m`,`📊 quality=${x.qualityScore}/100`)});}else lines.push('None in this cycle.');
  lines.push('',`🟡 RESEARCH NEAR-MISS (${nearMissResearchTop.length})`,`Criteria: complete history | exposure>=${MEME_NEARMISS_MIN_EXPOSURE}% | meme trades>=${MEME_NEARMISS_MIN_TRADES} | unique memes>=${MEME_NEARMISS_MIN_UNIQUE} | strict specialist NOT met`);
  if(nearMissResearchTop.length){nearMissResearchTop.forEach((x,i)=>{const p=x.meme||{},e=x.early||{};const missing=[Number(p.exposurePct||0)<MEME_MIN_EXPOSURE?'EXPOSURE':'',Number(p.memeTrades||0)<MEME_MIN_TRADES?'MEME_TRADES':'',Number(p.uniqueCoins||0)<MEME_MIN_UNIQUE?'UNIQUE_MEMES':''].filter(Boolean).join(', ')||'strict-only condition';lines.push(`#${i+1} ${x.address}`,`🧬 exposure=${pct(p.exposurePct,1)} | memeTrades=${p.memeTrades}/${p.totalTrades} | unique=${p.uniqueCoins}`,`🎯 dominant=${p.dominantMeme||'n/a'} (${p.dominantMemeTrades||0} trades, ${pct(p.dominantPct,1)}%)`,`🚀 early=${e.score||0}/100 | hit +5=${pct(e.hit5,0)} | +10=${pct(e.hit10,0)} | +20=${pct(e.hit20,0)} | lead5=${fmt(e.medianLead5,1)}m`,`📊 quality=${x.qualityScore}/100 | missing strict: ${missing}`)});}else lines.push('None in this cycle.');
  ...
  console.log(`[MEME-SCOUT V5.33][DONE] discovered=${d.discovered} scanned=${sourceAddresses.length} specialists=${scanned.length} top=${top.length} errors=${errors.length} seconds=${((Date.now()-t0)/1000).toFixed(1)}`);
  await telegram(lines.join('\n'));
}

main().catch(async e=>{console.error(`[MEME SCOUT V5.33][FATAL] ${e.stack||e}`);await telegram(`🟣 HYPERLIQUID MEME SPECIALIST SCOUT V5.33\n📡 READ-ONLY | NO ORDERS\n━━━━━━━━━━━━━━━━━━\n💥 FATAL ERROR\n${String(e.message||e).slice(0,1000)}`);process.exitCode=1});
