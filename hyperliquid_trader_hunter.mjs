...
async function main(){
 ...
 const eligible=enriched.filter(x=>x.plan?.eligible).sort((a,b)=>(b.finalScore-a.finalScore)||(b.statScore-a.statScore));
 const selected=AUTO_SELECT?eligible.slice(0,1):[];
 selected.forEach(x=>{x.autoSelected=true});
...
main().catch(async e=>{console.error(`[HUNTER V5.12][FATAL] ${e.stack||e}`);await telegram(`🟣 HYPERLIQUID TRADER HUNTER V5.12\n📡 READ-ONLY | NO ORDERS\n━━━━━━━━━━━━━━━━━━\n💥 FATAL ERROR\n${String(e.message||e).slice(0,1000)}`);process.exitCode=1});
