// Hyperliquid Meme Specialist Scout V5.32 - READ ONLY
...
// V5.32: strict specialist eligibility is preserved. A separate Focus tier
...
const MEME_FOCUS_MAX_DOMINANT_SHARE = num('HYPERLIQUID_MEME_FOCUS_MAX_DOMINANT_SHARE_PCT', 85);
const MEME_FOCUS_MIN_SPECIALIZATION = num('HYPERLIQUID_MEME_FOCUS_MIN_SPECIALIZATION_SCORE', 20);
...
function recentMemeProfile(fills){
  ...
  const focusCountScore=Math.min(100,meme.length/Math.max(1,MEME_FOCUS_MIN_TRADES)*100);
  const focusPotential=Math.round(.40*exposure+.25*focusCountScore+.25*breadth+.10*recency);
  const score=Math.round(.45*exposure+.20*countScore+.25*breadth+.10*recency);
  return {memeTrades:meme.length,totalTrades:total,exposurePct:exposure,uniqueCoins:unique,score,focusPotential};
}
...
async function fastMemePrefilter(addresses){
  ...
  const byFocusPotential=[...sorted].sort((a,b)=>b.profile.focusPotential-a.profile.focusPotential||b.profile.uniqueCoins-a.profile.uniqueCoins||b.profile.memeTrades-a.profile.memeTrades);
  ...
  add(byScore,Math.ceil(target*.35));
  add(byExposure,Math.ceil(target*.20));
  add(byCount,Math.ceil(target*.15));
  add(byBreadth,Math.ceil(target*.15));
  add(byFocusPotential,target);
  add(byScore,target);
...
function memeProfile(trades){
  ...
  const dominantShare=meme.length?dominant[1]/meme.length*100:0;
  const breadthScore=Math.min(100,unique/Math.max(1,MEME_FOCUS_MIN_UNIQUE)*100);
  const concentrationScore=Math.max(0,Math.min(100,100-dominantShare));
  return {memeTrades:meme.length,totalTrades:trades.length,exposurePct:exposure,uniqueCoins:unique,memeWinRate:wr,memeProfitFactor:pf,memePnl:pnl,totalPnl,medianHoldHours:holdMed,specializationScore:score,dominantMeme:dominant[0],dominantMemeTrades:dominant[1],dominantSharePct:dominantShare,breadthScore,concentrationScore,memeTradesList:meme};
}
...
async function analyzeMemeTrader(x,now){
  ...
  const focusEligible=p.memeTrades>=MEME_FOCUS_MIN_TRADES&&p.exposurePct>=MEME_FOCUS_MIN_EXPOSURE&&p.uniqueCoins>=MEME_FOCUS_MIN_UNIQUE&&p.specializationScore>=MEME_FOCUS_MIN_SPECIALIZATION&&p.dominantSharePct<=MEME_FOCUS_MAX_DOMINANT_SHARE;
  if(!focusEligible)return {...x,meme:p,early:{score:0,count:0,pump:0,dump:0},memeEligible:strictEligible,focusEligible:false,dataQualityGate:!incomplete};
  ...
  const focusScore=Math.round(.35*p.specializationScore+.25*edge+.15*p.breadthScore+.15*p.concentrationScore+.10*x.qualityScore);
  return {...x,meme:p,early:{...early,repeatability,score:edge},memeEligible:strictEligible,focusEligible:true,memeFocusScore:Math.max(0,Math.min(100,focusScore))};
}
...
async function main(){
  ...
  const errors=[];
  console.log(`[MEME-SCOUT V5.32][START] mode=${MEME_MODE} watchlist=${MEME_WATCHLIST.length}`);
  let d;
  try{d=await discover()}catch(e){
    console.error(`[DISCOVERY][ERROR] ${e.message}`);
    await telegram(`🟣 HYPERLIQUID MEME SPECIALIST SCOUT V5.32\n📡 READ-ONLY | NO ORDERS\n━━━━━━━━━━━━━━━━━━\n❌ DISCOVERY ERROR\n${e.message}`);process.exitCode=1;return;
  }
  ...
  lines.push('',`🟠 MEME-FOCUS CANDIDATES (${focusTop.length})`,`Criteria: exposure>=${MEME_FOCUS_MIN_EXPOSURE}% | meme trades>=${MEME_FOCUS_MIN_TRADES} | unique memes>=${MEME_FOCUS_MIN_UNIQUE} | dominant<=${MEME_FOCUS_MAX_DOMINANT_SHARE}% | spec>=${MEME_FOCUS_MIN_SPECIALIZATION}`);
  if(focusTop.length){focusTop.forEach((x,i)=>{const p=x.meme||{},e=x.early||{};lines.push(`#${i+1} ${x.address}`,`🧬 exposure=${pct(p.exposurePct,1)} | memeTrades=${p.memeTrades}/${p.totalTrades} | unique=${p.uniqueCoins}`,`🎯 dominant=${p.dominantMeme||'n/a'} (${p.dominantMemeTrades||0} trades) | share=${pct(p.dominantSharePct,1)}%`,`🧠 breadth=${Math.round(p.breadthScore||0)}/100 | concentration=${Math.round(p.concentrationScore||0)}/100 | focus=${x.memeFocusScore||0}/100`,`🚀 early=${e.score||0}/100 | hit +5=${pct(e.hit5,0)} | +10=${pct(e.hit10,0)} | +20=${pct(e.hit20,0)} | lead5=${fmt(e.medianLead5,1)}m`,`📊 quality=${x.qualityScore}/100`)});}else lines.push('None in this cycle.');
  ...
  console.log(`[MEME-SCOUT V5.32][DONE] discovered=${d.discovered} scanned=${sourceAddresses.length} specialists=${scanned.length} top=${top.length} errors=${errors.length} seconds=${((Date.now()-t0)/1000).toFixed(1)}`);
  await telegram(lines.join('\n'));
}

main().catch(async e=>{console.error(`[MEME SCOUT V5.32][FATAL] ${e.stack||e}`);await telegram(`🟣 HYPERLIQUID TRADER MEME SCOUT V5.32\n📡 READ-ONLY | NO ORDERS\n━━━━━━━━━━━━━━━━━━\n💥 FATAL ERROR\n${String(e.message||e).slice(0,1000)}`);process.exitCode=1});
