...
const CONFIG = {
VERSION: "V17.3.10-ONCHAIN-ALLOWANCE-HARDENING",
MODE: "SIGNAL",
...
async function ensureGmxCollateralAllowance(sdk,signer,account,symbol,requiredAmount,balances,options={}){
  ...
  const routerAddress=routerProbe?.to;
  if(!/^0x[0-9a-fA-F]{40}$/.test(String(routerAddress||"")))
    throw executionStageError("ERC20_APPROVAL_BUILD",new Error(`GMX_APPROVAL_ROUTER_ADDRESS_INVALID: ${String(routerAddress||"")}`),{token:symbol,tokenAddress});
  const rpcUrl=options?.rpcUrl;
  ...
  const verified=await waitForGmxRouterAllowance(sdk,account,symbol,required,30,{rpcUrl,tokenAddress,routerAddress:approveTx?.to||routerAddress});
