...
async function buildMarketRegimeDataCenter(sdk, markets, _broadRows = []) {
  ...
  async function analyzeAsset(asset) {
    ...
    const validFrames = ["1d","4h","1h"].filter(tf => frames[tf]?.available);
    const weights = { "1d":0.45, "4h":0.35, "1h":0.20 };
    const weightTotal = validFrames.reduce((sum, tf) => sum + weights[tf], 0);
    const score = weightTotal > 0
      ? Number((validFrames.reduce((sum, tf) => sum + Number(frames[tf].score ?? 50) * weights[tf], 0) / weightTotal).toFixed(1))
      : 50;
    return { asset, available:validFrames.length > 0, complete:validFrames.length === 3, direction, score,
      confidence:Number(Math.abs(score - 50).toFixed(1)), frames, bullishFrames, bearishFrames,
      availableFrames:validFrames.length, timeframeAgreement:Number((Math.max(bullishFrames,bearishFrames)/Math.max(1,validFrames.length)*100).toFixed(1)) };
  }

  const assets = {};
  for (const asset of CONFIG.referenceAssets) assets[asset] = await analyzeAsset(asset);

  const usable = Object.values(assets).filter(x => x.available && x.complete);
  if (usable.length < 3) {
    const available = Object.values(assets).filter(x => x.available);
    return { enabled:true, regime:"DATA_INCOMPLETE", direction:"neutral", score:50, confidence:0, assets,
      agreement:0, reason:"REFERENCE_TIMEFRAME_DATA_INCOMPLETE", availableAssets:available.length,
      requiredAssets:3, timeframe:"D1/H4/H1", timestamp:Date.now() };
  }
